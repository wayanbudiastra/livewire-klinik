<div>
    ...
    <div class="mb-4 flex flex-col sm:flex-row gap-3 justify-between">
        ...
        <div class="flex gap-2">
            {{-- Tombol Ambil Semua IHS Perawat --}}
            @if($satusehatAktif && $filterRole === 'perawat')
            <button wire:click="fetchIhsSemua"
                ...
                <span wire:loading.remove wire:target="fetchIhsSemua">Ambil Semua IHS Perawat</span>
    ...
    {{-- Progress IHS bulk --}}
    @if($satusehatAktif && $filterRole === 'perawat' && ($ihsRunning || $ihsSelesai))
    <div class="mb-4 rounded-xl border p-4 text-sm
        ...
        <div class="flex items-center justify-between mb-2">
            <span class="font-medium {{ $ihsSelesai ? 'text-emerald-700 dark:text-emerald-400' : 'text-blue-700 dark:text-blue-400' }}">
                {{ $ihsSelesai ? '✓ Selesai' : 'Mengambil IHS Perawat...' }}
            </span>
            @if($ihsSelesai)
            ...
            <tbody>
                ...
                <tr wire:key="user-{{ $user->id }}">
                    ...
                    <td>
                        ...
                        @endif
                        @if($filterRole === 'perawat' && $user->perawat?->nik)
                            <p class="text-xs text-gray-400 font-mono">NIK: {{ $user->perawat->nik }}</p>
                        @endif
                    </td>
                    ...
                    {{-- IHS Status (hanya tampil jika filter perawat + satusehat aktif) --}}
                    @if($satusehatAktif && $filterRole === 'perawat')
                    <td>
                        @if($user->perawat)
                            @if($user->perawat->ihs_status === 'ditemukan')
                                <span class="inline-flex items-center gap-1 text-xs text-emerald-700 dark:text-emerald-400 font-mono">
                                    ...
                                    {{ Str::limit($user->perawat->ihs_id, 10) }}
                                </span>
                            @elseif($user->perawat->ihs_status === 'tidak_ditemukan')
                                <span class="text-xs text-amber-600">Tidak ditemukan</span>
                            @elseif($user->perawat->ihs_status === 'error')
                                <span class="text-xs text-red-500" title="{{ $user->perawat->ihs_error_msg }}">Error</span>
                            @elseif($user->perawat->nik)
                                <span class="text-xs text-gray-400">Belum diambil</span>
                            @else
                                <span class="text-xs text-amber-500">NIK kosong</span>
                            @endif
                        @else
                            <span class="text-xs text-gray-300">Profil belum ada</span>
                        @endif
                    </td>
                    @endif
                    ...
                    <td>
                        <div class="flex items-center gap-1">
                            ...
                            @if($satusehatAktif && $filterRole === 'perawat')
                            <button wire:click="fetchIhsSatu({{ $user->id }})"
                                    wire:loading.attr="disabled"
                                    wire:target="fetchIhsSatu({{ $user->id }})"
                                ...
                                <svg wire:loading.remove wire:target="fetchIhsSatu({{ $user->id }})" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                                </svg>
                                <svg wire:loading wire:target="fetchIhsSatu({{ $user->id }})" class="animate-spin w-3.5 h-3.5" fill="none" viewBox="0 0 24 24">
                                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
                                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
                                </svg>
                            </button>
                ...
                @empty
                <tr>
                    <td colspan="{{ ($satusehatAktif && $filterRole === 'perawat') ? 7 : 6 }}">
